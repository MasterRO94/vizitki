...
/*=================== Save Order =====================*/
    function saveOrder($order){
        ...
        $q = "INSERT INTO `orders` (`id`, `user_id`, `type`, `tiraj`, `kolvo`, `type_sides`, `wishes`, `paper_type`, `image_face`, `image_back`, `dop_uslugi`, `created_at`, `updated_at`, `status`, `totalSum`) VALUES(NULL, '{$user['user_id']}', '{$order['type']}', '{$order['tiraj']}', '{$order['kolvo']}', '{$order['type_sides']}', '{$order['wishes']}', '{$order['paper_type']['id']}', '".$order['image_face']."', '".$order['image_back']."', '{$order['dop_uslugi']}', NOW(), NOW(), 'Новый', {$order['totalSum']})";
        $result = mysqli_query($link,$q);
        if(!$result){
            return false;
        }
